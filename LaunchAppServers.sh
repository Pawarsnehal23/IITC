 ...
 if [ $# != 8 ]
    then 
    echo "This script needs 8 arguments/variables to run; ELB-NAME, KEYPAIR, CLIENT-TOKENS, NUMBER OF INSTANCES, and SECURITY-GROUP-NAME"
 else
	...
	echo "Creating load balancer in AWS ";
	
    ELBURL=$(aws elb create-load-balancer  --load-balancer-name  $6  --listeners Protocol=HTTP,LoadBalancerPort=80,InstanceProtocol=HTTP,InstancePort=80  --security-groups $GRPID  --subnets $SUBNETID);
 
	echo -e "\nFinished launching ELB and sleeping 25 seconds"
	for i in {0..25}; do echo -ne '.'; sleep 1;done

	
	echo "DNS for load balancer is - " $ELBURL ;
	
	#apply session stickiness policy to load balancer
	echo -e "\nApplying session stickiness policy to ELB."
	ELBPOLICY=$(aws elb create-lb-cookie-stickiness-policy --load-balancer-name  $6 --policy-name 'sessionStickinessPolicy');
	
	#ELB health check
	aws elb configure-health-check --load-balancer-name  $6 --health-check Target=HTTP:80/Index.php,Interval=30,UnhealthyThreshold=2,HealthyThreshold=2,Timeout=3
	...
	echo "Registering  instances to load balancer in AWS ";

	QUERY_RESULTS=$(aws ec2 describe-instances --output=text | grep $1 | awk {'print $8'});
	
	echo $QUERY_RESULTS
	for i in `echo $QUERY_RESULTS | tr ' ' '\n'`
	  do
	  echo "Registering " $i " to load balancer " $6 
	  aws elb register-instances-with-load-balancer --load-balancer-name $6   --instance $i  --output=text ;
	done

	
	#Last Step
	firefox -new-tab $ELBURL
